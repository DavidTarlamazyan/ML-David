...
for data in face_data:
	if "name" in data and "face_encoding" in data and "access" in data:
		kown_face_encodings.append(data['face_encoding'])
...
while True:
	...
	for (x,y,w,h) in face_rects:
		...
		face_encoding = face_recognition.face_encodings(rhb_frame. [(y, x+w, y+h, x)])[0]
	matched_names = []
	distances = face_recognition.face_distance(known_face_encodings, face encoding)
