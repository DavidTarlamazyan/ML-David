...
acces_input = ("Enter room access (comma-separated): ")
acces_list = access_input.split(',')
...
while True:
	...
	for(x,y,w,h) in faces:
		...
		for face_encoding in face_encodings:
			face_data.append({"name": name, "face": frame[y:y+h, x:x+w], "face_encoding": face_encoding, "access": acces_list})
	cv2.imshow('Register frame', frame)
	if cv2.waltKey(1) & 0xFF == ord('s'):
		capture_count += 1
